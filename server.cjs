...
function sig(secret, raw){ return crypto.createHash('sha1').update(secret + '|' + raw, 'utf8').digest('hex').slice(0,8); }

app.get('/', (_req,res)=>res.type('text/plain').send('OK'));

app.post('/api/register', (req,res)=>{
  const raw = req.rawBody || '';
  const got = (req.get('X-Sig')||'').trim().toLowerCase();
  const want = sig(SHARED_SECRET, raw);
  if(!got) return res.status(400).json({ok:false,error:'Missing X-Sig header'});
